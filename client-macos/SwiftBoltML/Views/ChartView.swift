...
// MARK: - Multi-Timeframe Forecast Grid View (Improvements 6-7)
...
// MARK: - Data Quality Badge (Improvement 4)
...
struct DataQualityBadge: View {
    ...
    var body: some View {
        if let quality = dataQuality {
            ...
            .help(quality.statusDescription)
        }
    }

    private var statusColor: Color {
        guard let quality = dataQuality else { return .gray }
        if quality.isStale { return .orange }
        if quality.hasRecentData { return .green }
        return .yellow
    }

    private var statusText: String {
        guard let quality = dataQuality else { return "Unknown" }
        if quality.isStale { return "Stale" }
        if quality.hasRecentData { return "Fresh" }
        return "Recent"
    }
}
