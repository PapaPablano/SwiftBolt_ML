    ...
    var body: some View {
        ...
        return VStack(spacing: 0) {
            ...
            HStack {
                ...
                IndicatorToggleMenu(chartViewModel: chartViewModel, config: deferredBinding(get: { chartViewModel.indicatorConfig }, set: { chartViewModel.indicatorConfig = $0 }))
            }
            .padding(.horizontal)
            .padding(.top, 8)
    ...
    var body: some View {
        Menu {
            Section("Chart Options") {
                Toggle("Heikin-Ashi", isOn: deferredBinding(get: { chartViewModel.useHeikinAshi }, set: { chartViewModel.useHeikinAshi = $0 }))
                    .disabled(!config.useWebChart)
                Toggle("Volume Profile", isOn: deferredBinding(get: { chartViewModel.showVolumeProfile }, set: { chartViewModel.showVolumeProfile = $0 }))
                    .disabled(!config.useWebChart)
            }
