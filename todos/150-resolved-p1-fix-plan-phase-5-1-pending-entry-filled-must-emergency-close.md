...
- [x] Phase 5.1 `pending_entry` + filled branch changed to `closeLivePosition`, not `update({status: 'open'})`
- [x] Comment explains that current_price fallback to entry_price is expected and P&L needs reconciliation
- [x] `pending_entry` + unfilled branch (order cancelled/rejected) still cancels the position
- [x] Behavior is symmetric with the `pending_bracket` branch
