...
- [ ] `disconnect_broker` backend checks for open positions and returns 409 if any exist
- [ ] Frontend "Disconnect Broker" button shows confirmation dialog warning about SL/TP monitoring
- [ ] If no open positions exist, disconnect proceeds as before
- [ ] 409 response body includes `open_position_count` so the frontend can show a specific count
