...
- [x] `'EMERGENCY_CLOSE'` added to `live_trading_positions.close_reason` CHECK constraint
- [x] Phase 5's recovery scan uses the same casing as the CHECK constraint (coordinate with #154)
- [x] Test: attempt to INSERT a position with `close_reason = 'EMERGENCY_CLOSE'` — should succeed
- [x] No duplicate constraint names — old constraint is dropped before new one is added
