...
- [x] `increment_rate_limit` function uses two-statement form (upsert + SELECT)
- [x] `CREATE OR REPLACE FUNCTION` succeeds without syntax errors on Postgres 15
- [x] Function returns TRUE when under the limit, FALSE when at or over the limit
- [x] GRANT EXECUTE included (see todo #157)
