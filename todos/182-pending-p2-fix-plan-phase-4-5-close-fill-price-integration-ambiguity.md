...
- [ ] Plan specifies whether `pollOrderFill` is called inside `closeLivePosition` or in the handler
- [ ] Plan shows the complete updated `closeLivePosition` signature
- [ ] If Option B, plan shows all call sites updated to pass the fill price
- [ ] The trade record's `exit_price` is confirmed to be the actual fill price, not `pos.current_price`
