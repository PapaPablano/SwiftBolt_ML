...
- [x] Phase 4.2 insert-error recovery applies `!cancelled.ok` check to the `cancelOrder` call
- [x] Plan documents what operators should do when `cancelOrder` fails in this path (since the recovery scan cannot detect the order)
- [x] Plan notes that this is a different issue from the double-cancel bug in todo #149
