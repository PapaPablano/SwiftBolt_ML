...
- [ ] Plan documents whether TradeStation `BRK` order type is OCO (confirmed via SIM test or API docs)
- [ ] If not OCO: `cancelOrder` added for opposite leg in both SL-fill and TP-fill branches
- [ ] Plan stores `broker_sl_order_id` and `broker_tp_order_id` in `live_trading_positions` so opposite-leg IDs are available at fill detection time
- [ ] `cancelOrder` failure on opposite-leg cancel is logged as an alert (not silent)
