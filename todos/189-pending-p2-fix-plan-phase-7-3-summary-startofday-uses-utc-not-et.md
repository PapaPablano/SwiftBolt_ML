...
- [ ] Phase 7.3 `startOfDay` uses Eastern Time, not UTC
- [ ] `getETStartOfDay()` utility extracted and used by both `handleSummary` and `checkDailyLossLimit`
- [ ] DST handled correctly via `America/New_York` timezone (not hardcoded -05:00 offset)
- [ ] Test: trades closed at 11pm ET appear in the same day's summary, not the next day's
