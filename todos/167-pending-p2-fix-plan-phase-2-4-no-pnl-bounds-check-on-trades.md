...
- [ ] `live_trading_trades.pnl` has a CHECK constraint bounding it to a realistic range
- [ ] Constraint is tight enough to catch multiplier-of-50x errors on micro lots
- [ ] `pnl_pct` bounds added if the column is populated
- [ ] Plan documents what the bounds represent (maximum 10% position, maximum plausible equity)
