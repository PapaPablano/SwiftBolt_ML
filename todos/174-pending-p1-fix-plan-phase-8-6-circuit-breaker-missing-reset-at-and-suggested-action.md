...
- [ ] All four circuit breaker rules include `reset_at` (ISO 8601 or null) in the response
- [ ] All four circuit breaker rules include `suggested_action` (typed string literal) in the response
- [ ] `nextMarketOpen()` is extracted as a reusable utility from `checkMarketHours()`
- [ ] Plan documents the valid `suggested_action` values and their meanings
- [ ] `reset_at` is null for `max_positions` and `position_size_cap` (non-deterministic reset)
