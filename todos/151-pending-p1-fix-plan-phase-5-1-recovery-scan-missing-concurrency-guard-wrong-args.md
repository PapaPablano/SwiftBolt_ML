...
- [ ] `recoverStuckPositions` uses optimistic status transition before any broker call
- [ ] `closeLivePosition` called with all 7 required arguments (supabase, token, accountId, position, price, reason, tsSymbol)
- [ ] `"closing_emergency"` added to `status` CHECK enum OR inline cancellation logic avoids the need for it
- [ ] Zero risk of double-close under concurrent invocations
