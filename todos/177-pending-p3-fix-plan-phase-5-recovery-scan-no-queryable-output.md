...
- [ ] Phase 5 in the plan includes a note documenting that recovery events are not queryable via the GET API
- [ ] Plan flags as a follow-up: add `close_reason` filter to trades endpoint OR add `live_trading_recovery_events` table
- [ ] If `close_reason` filter is added in this PR, document it in Phase 8.4 alongside the other filter additions
