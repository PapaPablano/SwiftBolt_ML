...
- [x] `REVOKE EXECUTE FROM PUBLIC, anon, authenticated` added to Phase 2.2 RPC definition
- [x] `GRANT EXECUTE TO service_role` added
- [x] `auth.uid() = p_user_id` validation added inside function body with clear exception message
- [x] Plan notes that the executor must use service_role key (not anon key) to call this RPC
...
- 2026-03-03: Resolved — plan amended with security controls, acceptance criteria updated.
