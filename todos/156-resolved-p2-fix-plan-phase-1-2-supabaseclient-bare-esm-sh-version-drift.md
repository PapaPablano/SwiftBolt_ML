...
- [x] `SupabaseClient` type import uses the same specifier as `createClient` (import map, not bare esm.sh)
- [x] `deno check` passes with no type errors on both files
- [x] No version drift between the type and value imports
