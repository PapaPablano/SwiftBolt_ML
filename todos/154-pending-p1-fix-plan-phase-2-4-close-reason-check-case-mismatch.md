...
- [ ] `live_trading_trades.close_reason` CHECK constraint values match the casing the executor writes
- [ ] `EMERGENCY_CLOSE` and `CIRCUIT_BREAKER` added as valid values for new recovery/circuit-breaker paths
- [ ] Existing `live_trading_positions.close_reason` CHECK values remain unchanged (or are verified consistent)
- [ ] Trade inserts from all close paths pass the constraint without error
