...
- [x] `handleClose` checks `result.success` after successful HTTP response
- [x] `{ success: false }` response shows the same error toast as an HTTP error
- [x] Pattern applied consistently to all executor POST operations in the frontend
- [x] Plan notes that the executor can return HTTP 200 with `success: false`
