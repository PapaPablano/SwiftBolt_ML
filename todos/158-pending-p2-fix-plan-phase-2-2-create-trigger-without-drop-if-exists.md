...
- [ ] `DROP TRIGGER IF EXISTS live_positions_updated_at ON live_trading_positions` added before the `CREATE TRIGGER`
- [ ] Migration can be applied multiple times without error (idempotent)
- [ ] Matches the drop-then-create convention used in existing migrations
