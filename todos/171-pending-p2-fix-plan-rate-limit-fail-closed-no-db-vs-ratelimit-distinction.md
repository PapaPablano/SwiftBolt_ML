...
- [ ] Phase 3 distinguishes `"rate_limited"` (genuine) from `"db_error"` (fail-closed) in the RateLimitResult type
- [ ] DB errors return HTTP 503 with `retryable: true`, not 429
- [ ] Genuine rate limits return HTTP 429 with `Retry-After: 60`
- [ ] Structured log events use different keys (`rate_limit_exceeded` vs `rate_limit_rpc_failed`) for operational alerting
- [ ] Plan notes that fail-closed is intentional for safety, but must be distinguishable in logs
