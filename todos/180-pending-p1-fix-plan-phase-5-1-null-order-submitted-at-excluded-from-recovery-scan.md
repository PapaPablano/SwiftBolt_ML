...
- [ ] Recovery query uses `.or("order_submitted_at.lt.X,order_submitted_at.is.null")` pattern
- [ ] NULL positions are treated as "stuck since unknown time" and included in recovery
- [ ] Plan optionally adds NOT NULL DEFAULT NOW() constraint to prevent future NULL insertions
- [ ] Test case documented: insert a position with NULL `order_submitted_at`, verify recovery scan finds it
