...
- [ ] `GET ?action=rate_limit_status` returns `{ requests_used, requests_remaining, limit, window_resets_at }`
- [ ] Endpoint is read-only and does not count against the rate limit
- [ ] `window_resets_at` is a valid ISO 8601 timestamp marking when the current window expires
- [ ] Plan documents this endpoint as the recommended pre-execution check for rate-limit-aware agents
