...
- [x] `"FLP"` changed to `"FPR"` in the `OrderFillResult` status union
- [x] Plan documents that `| string` is intentional (broker may send undocumented status codes)
- [x] All existing partial fill checks in the executor use `"FPR"` consistently
