...
- [x] `?strategy_id=UUID` filter supported on GET `?action=positions`
- [x] Filter applied server-side (not client-side) using `idx_live_positions_user_strategy` index
- [x] `?status=` and `?strategy_id=` filters can be combined in a single request
- [x] Plan documents `GET ?action=positions&status=open&strategy_id=<uuid>` as the recommended agent pre-execution check
