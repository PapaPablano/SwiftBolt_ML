...
- [ ] DROP INDEX targets correct names (`idx_live_positions_active_unique`, `idx_live_positions_user_strategy`)
- [ ] Recreated indexes use original names
- [ ] `idx_live_positions_active_unique` unique partial index exists after migration with TEXT-typed `symbol_id`
- [ ] No duplicate index definitions on the same column set
- [ ] `EXPLAIN` on a sample query still uses the index
