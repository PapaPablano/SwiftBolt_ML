-- Migration: Fix volume type casting in get_chart_data_v2
-- Problem: SUM(volume) returns NUMERIC, but function expects BIGINT
-- Solution: Explicit ::BIGINT cast on all aggregated volume columns
  ...
  IF p_timeframe IN ('m15', 'h1', 'h4') THEN
    IF p_timeframe = 'm15' THEN
      ...
      SELECT to_char(ib.ts AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"')::TEXT,
        ...
        ib.volume::BIGINT, 'tradier'::VARCHAR(20), true::BOOLEAN, false::BOOLEAN, 'verified'::VARCHAR(20),
    ...
    ELSIF p_timeframe = 'h1' THEN
      ...
      SELECT to_char(ha.hour_ts AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"')::TEXT,
        ...
        ha.volume, 'tradier'::VARCHAR(20), true::BOOLEAN, false::BOOLEAN, 'verified'::VARCHAR(20),
        NULL::DECIMAL(3,2), NULL::DECIMAL(10,4), NULL::DECIMAL(10,4)
    ...
    ELSIF p_timeframe = 'h4' THEN
      ...
      SELECT to_char(fha.four_hour_ts AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"')::TEXT,
        ...
        fha.volume, 'tradier'::VARCHAR(20), true::BOOLEAN, false::BOOLEAN, 'verified'::VARCHAR(20),
        NULL::DECIMAL(3,2), NULL::DECIMAL(10,4), NULL::DECIMAL(10,4)
...
'Returns chart data with proper volume type casting.
Fixed: SUM(volume)::BIGINT to match function signature.
