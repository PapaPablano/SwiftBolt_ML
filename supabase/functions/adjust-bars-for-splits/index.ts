...
serve(async (req) => {
  try {
    ...
    for (const split of splits) {
      console.log(`[adjust-bars] Processing ${split.symbol} split on ${split.date} (${split.ratio}:1)`);
      ...
      if (caError || !corporateAction) {
        console.error(`[adjust-bars] Corporate action not found for ${split.symbol}:`, caError);
        continue;
      }
      ...
      if (symbolError || !symbolData) {
        console.error(`[adjust-bars] Symbol not found: ${split.symbol}`, symbolError);
        continue;
      }
      ...
      if (barsError) {
        console.error(`[adjust-bars] Error fetching bars for ${split.symbol}:`, barsError);
        continue;
      }
      
      if (!bars || bars.length === 0) {
        console.log(`[adjust-bars] No unadjusted bars found for ${split.symbol}`);
      ...
      console.log(`[adjust-bars] Adjusting ${bars.length} bars for ${split.symbol} with ratio ${adjustmentRatio}`);
      ...
      for (let i = 0; i < adjustedBars.length; i += chunkSize) {
        ...
        if (updateError) {
          console.error(`[adjust-bars] Error updating bars chunk ${i}-${i + chunk.length}:`, updateError);
        }
      }
      ...
      console.log(`[adjust-bars] Successfully adjusted ${bars.length} bars for ${split.symbol}`);
