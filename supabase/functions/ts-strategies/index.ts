...
async function handleStrategies(
  ...
  if (method === 'GET') {
    const { data, error } = await supabase
      .from('ts_strategies')
      .select('*, ts_strategy_conditions(*, ts_indicators(*)), ts_trading_actions(*)')
      .eq('user_id', userId)
...
async function handleAuth(req: Request, supabase: any, userId: string) {
  ...
  if (method === 'POST') {
    ...
    if (action === 'exchange') {
      ...
      return new Response(JSON.stringify({ success: true, expires_in: tokens.expires_in }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
    ...
    if (action === 'status') {
      ...
      const isExpired = creds?.token_expires_at ? new Date(creds.token_expires_at) < new Date() : true

      return new Response(JSON.stringify({ connected: isConnected, expired: isExpired }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
  }
...
async function handleExecute(req: Request, supabase: any, userId: string) {
  ...
  if (!creds?.access_token) {
    return new Response(JSON.stringify({ error: 'Not authenticated with TradeStation' }), {
      status: 401,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    })
  }
  ...
  if (!quoteResponse.ok) {
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch quote', details: error }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    })
  }
  ...
  for (const condition of conditions) {
    ...
    const indicatorValue = quote[condition.indicators?.name?.toLowerCase()] || 0
    const threshold = condition.threshold
    
    switch (condition.operator) {
      case '>': conditionsMet = conditionsMet && indicatorValue > threshold; break
      case '<': conditionsMet = conditionsMet && indicatorValue < threshold; break
      case '>=': conditionsMet = conditionsMet && indicatorValue >= threshold; break
      case '<=': conditionsMet = conditionsMet && indicatorValue <= threshold; break
      case '=': conditionsMet = conditionsMet && indicatorValue === threshold; break
