...
addEventListener("unhandledrejection", (event) => {
  console.error(
    "[chart-data-v2] Unhandled rejection:",
...
const ALL_TIMEFRAMES: Array<"m15" | "h1" | "h4" | "d1" | "w1"> = [
  "m15",
  "h1",
  "h4",
  "d1",
...
function buildIntradayForecastPoints(params: {
  ...
  for (let i = 1; i <= safeSteps; i += 1) {
    const t = i / safeSteps;
    const value = params.currentPrice +
      (params.targetPrice - params.currentPrice) * t;
...
function buildForecastBarsFromSummary(summary: unknown): ChartBar[] {
  ...
  if (isDailyMultiHorizon) {
    for (const horizon of horizons) {
      ...
      const targetPoint = points.reduce(
        (latest, current) => (current.ts > latest.ts ? current : latest),
        points[0],
      );
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (futuresRootPattern.test(requestedSymbol)) {
      ...
      if (rootCheck) {
        return new Response(
          JSON.stringify({
            error:
              `Futures root "${requestedSymbol}" requires expiry selection. Please select a specific contract (e.g., ${requestedSymbol}Z25) or continuous alias (e.g., ${requestedSymbol}1!).`,
    ...
    if (futuresPattern.test(requestedSymbol)) {
      console.log(
        `[chart-data-v2] Detected futures symbol: ${requestedSymbol}`,
      );
      ...
      try {
        ...
        if (!resolveError && resolved && resolved.length > 0) {
          ...
          console.log(
            `[chart-data-v2] Resolved ${requestedSymbol} → ${resolvedSymbol} (continuous: ${isContinuousFutures})`,
          );
        } else {
          console.log(
            `[chart-data-v2] Could not resolve futures symbol ${requestedSymbol}, using as-is`,
          );
        }
      } catch (err) {
    ...
    if (alpacaApiKey && alpacaApiSecret) {
      ...
      for (const tf of ALL_TIMEFRAMES) {
        ...
        const latestIso = latestBar?.ts
          ? new Date(latestBar.ts).toISOString()
          : null;
        const latestAgeSec = latestIso
          ? (Date.now() - new Date(latestIso).getTime()) / 1000
          : Number.POSITIVE_INFINITY;
        const maxAgeSec = timeframeMaxAgeSeconds(tf);
        const weeklyStale = tf === "w1" && !isWeeklyBarCurrent(latestIso);

        if (
          !Number.isFinite(latestAgeSec) || latestAgeSec > maxAgeSec ||
          weeklyStale
        ) {
          try {
            const startIso = latestIso
              ? new Date(new Date(latestIso).getTime() - intervalSec * 1000 * 2)
                .toISOString()
              : new Date(
            ...
            const isWeeklyStale = tf === "w1" &&
              latestIso &&
              (!newestAlpacaIso ||
                new Date(newestAlpacaIso).getTime() <=
            ...
            if (alpacaBars.length > 0 && !isWeeklyStale) {
              ...
              const { error: upsertError } = await supabase.from("ohlc_bars_v2")
                .upsert(rows, {
              ...
              const weeklyStartIso = latestIso ??
                new Date(
                  new Date().setUTCFullYear(new Date().getUTCFullYear() - 2),
                ).toISOString();
              ...
              if (weeklyBars.length > 0) {
                ...
                const { error: weeklyUpsertError } = await supabase.from(
                  "ohlc_bars_v2",
                ).upsert(rows, {
                  onConflict: "symbol_id,timeframe,ts,provider,is_forecast",
                });
            ...
            console.error(
              `[chart-data-v2] Alpaca refresh failed for ${tf}:`,
              alpacaErr,
            );
          }
        }
    ...
    console.log(
      `[chart-data-v2] Fetching: symbol_id=${symbolId}, timeframe=${timeframe}, start=${startDate.toISOString()}, end=${endDate.toISOString()}`,
    );
    ...
    if (timeframe === "d1" && chartData && chartData.length > 0) {
      console.log(
        "[chart-data-v2] d1 timeframe detected - fetching h1 data for intraday layer",
      );
      ...
      if (!h1Error && isValidChartBarArray(h1Data)) {
        ...
        console.log(
          `[chart-data-v2] Fetched ${intradayHourlyData.length} h1 bars for d1 intraday layer`,
        );
      } else {
        console.log(
          "[chart-data-v2] Could not fetch h1 data for d1 intraday layer",
        );
      }
    }
    ...
    if (chartError) {
      ...
      console.error(
        "[chart-data-v2] Error details:",
        JSON.stringify(chartError, null, 2),
      );
      return new Response(
        JSON.stringify({
    ...
    if (chartData === null) {
      const fallbackError = buildChartError(
        "No chart data returned from either dynamic or legacy RPC",
      );
    ...
    console.log(
      `[chart-data-v2] Success: received ${chartData.length} bars (maxBars=${maxBars})`,
    );
    ...
    if (chartData && chartData.length > 0) {
      ...
      console.log(
        `[chart-data-v2] DEBUG: Oldest bar: ${oldest.ts} (${oldest.provider})`,
      );
      console.log(
        `[chart-data-v2] DEBUG: Newest bar: ${newest.ts} (${newest.provider})`,
      );

      const newestDate = new Date(newest.ts);
      const now = new Date();
      const ageHours = (now.getTime() - newestDate.getTime()) /
        (1000 * 60 * 60);
      console.log(
        `[chart-data-v2] DEBUG: Newest bar age: ${ageHours.toFixed(1)} hours`,
      );

      if (ageHours > 24) {
        console.warn(
          `[chart-data-v2] ⚠️ WARNING: Newest bar is ${
    ...
    if (timeframe === "d1" && intradayHourlyData.length > 0) {
      console.log(
        `[chart-data-v2] Using h1 data (${intradayHourlyData.length} bars) as intraday layer for d1`,
      );
        ...
        if (isIntraday) {
          ...
          if (
            ...
            indicators = {
              ...
              trendLabel: intradayPath?.overall_label ??
                intradayForecast.overall_label,
            ...
            if (!Number.isFinite(targetPrice)) {
              throw new Error(
                "[chart-data-v2] intradayForecast.target_price is not a finite number",
              );
            }

            const currentPrice = clampNumber(
              intradayForecast.current_price,
              targetPrice,
            );
            const conf = clampNumber(intradayForecast.confidence, 0.5);
            ...
            const shortPoints = intervalSec
              ...
              : [{
                ts: Math.floor(
                  new Date(intradayForecast.expires_at).getTime() / 1000,
                ),
                value: targetPrice,
            ...
            const sampledShortPoints = sampleForecastPoints(
              shortPoints,
              INTRADAY_FORECAST_MAX_POINTS,
            );
          ...
          if (Array.isArray(dailyForecasts)) {
            for (const row of dailyForecasts) {
              const horizon = typeof row?.horizon === "string"
                ? row.horizon
                : null;
          ...
          if (dailySeries.length > 0) {
            ...
            const dailySummary = {
              overallLabel: typeof bestForecast?.overall_label === "string"
                ? bestForecast.overall_label
                : null,
              ...
              srLevels: (bestForecast?.sr_levels as Record<string, unknown>) ||
                null,
              srDensity: typeof bestForecast?.sr_density === "number"
                ? bestForecast.sr_density
                : null,
            };

            if (mlSummary) {
              const existing = new Set(
                (mlSummary.horizons || []).map((h: { horizon: string }) =>
              ...
              mlSummary = {
                ...
                overallLabel: mlSummary.overallLabel ??
                  dailySummary.overallLabel,
          ...
          if (horizonSeries.length > 0) {
            ...
            indicators = {
              supertrendFactor:
                typeof bestForecast?.supertrend_factor === "number"
                  ? bestForecast.supertrend_factor
                  : null,
              supertrendPerformance:
                typeof bestForecast?.supertrend_performance === "number"
                  ? bestForecast.supertrend_performance
                  : null,
              supertrendSignal:
                typeof bestForecast?.supertrend_signal === "number"
                  ? bestForecast.supertrend_signal
                  : null,
              trendLabel: typeof bestForecast?.trend_label === "string"
                ? bestForecast.trend_label
                : null,
              trendConfidence:
                typeof bestForecast?.trend_confidence === "number"
                  ? bestForecast.trend_confidence
                  : null,
              stopLevel: typeof bestForecast?.stop_level === "number"
                ? bestForecast.stop_level
                : null,
              trendDurationBars:
                typeof bestForecast?.trend_duration_bars === "number"
                  ? bestForecast.trend_duration_bars
                  : null,
              rsi: typeof bestForecast?.rsi === "number"
                ? bestForecast.rsi
                : null,
              adx: typeof bestForecast?.adx === "number"
                ? bestForecast.adx
                : null,
              macdHistogram: typeof bestForecast?.macd_histogram === "number"
                ? bestForecast.macd_histogram
                : null,
              kdjJ: typeof bestForecast?.kdj_j === "number"
                ? bestForecast.kdj_j
                : null,
            };
          }
    ...
    const newestBar = chartData && chartData.length > 0
    ...
    const dataQuality = {
      dataAgeHours: newestBar
        ? Math.round(
          (new Date().getTime() - new Date(newestBar.ts).getTime()) /
            (1000 * 60 * 60),
        )
        : null,
      isStale: newestBar
      ...
      historicalDepthDays: (oldestBar && newestBar)
        ? Math.round(
          (new Date(newestBar.ts).getTime() -
            new Date(oldestBar.ts).getTime()) / (1000 * 60 * 60 * 24),
        )
        : 0,
    ...
    if (symbolData?.asset_type === "future") {
      futuresMetadata = {
        ...
        is_dated_contract: !isContinuousFutures &&
          symbolData?.expiry_month != null,
          ...
          ? {
            ...
            display: `${
              [
                ...
                "Nov",
                "Dec",
              ][symbolData.expiry_month - 1]
            } ${symbolData.expiry_year}`,
          }
          : null,
      };
    }
    ...
    const response = {
      ...
      layers: {
        ...
        intraday: {
          ...
          newestBar: intraday.length > 0
            ? intraday[intraday.length - 1].ts
            : null,
        },
        forecast: {
          ...
          newestBar: forecast.length > 0
            ? forecast[forecast.length - 1].ts
            : null,
        },
      },
      metadata: {
        ...
        forecast_steps: typeof forecastSteps === "number"
          ? Math.floor(forecastSteps)
          : null,
      },
