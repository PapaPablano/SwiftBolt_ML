...
serve(async (req) => {
  try {
    ...
    console.log(
      `[sync-corporate-actions] Found ${actions.length} corporate actions`,
    );
    ...
    for (const action of actions) {
      ...
      const { error: upsertError } = await supabase.from("corporate_actions")
        .upsert({
      ...
      if (!upsertError) {
        ...
        console.error(
          `[sync-corporate-actions] Error upserting ${action.symbol}:`,
          upsertError,
        );
      }
    }
    ...
    if (splits.length > 0) {
      console.log(
        `[sync-corporate-actions] Found ${splits.length} splits, triggering adjustment job`,
      );
      ...
      if (invokeError) {
        console.error(
          "[sync-corporate-actions] Error invoking adjustment job:",
          invokeError,
        );
      }
    }
