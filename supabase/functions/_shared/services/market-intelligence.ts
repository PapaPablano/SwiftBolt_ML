...
export type CorporateActionType = 'stock_split' | 'reverse_split' | 'dividend' | 'merger' | 'spin_off';
...
export class MarketIntelligence {
  ...
  async getCorporateActions(
    ...
    return actions.map(action => ({
      ...
      type: action.ca_type,
      date: action.ex_date,
      ratio: action.new_rate && action.old_rate ? action.new_rate / action.old_rate : undefined,
      amount: action.cash,
      metadata: action
    }));
  }
