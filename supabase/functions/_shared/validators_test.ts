...
Deno.test('validateMarketData: Null OHLC values', () => {
  const barsWithNulls: any[] = [
    { time: '2026-01-01', open: null, high: 105, low: 99, close: 102, volume: 1000 },
    { time: '2026-01-02', open: 102, high: 108, low: 101, close: 105, volume: 1200 },
  ];
  const result = validateMarketData(barsWithNulls);
  assertEquals(result.valid, false);
  assertEquals(result.errors.some(e => e.includes('Open price is null')), true);
});
...
Deno.test('validateMarketData: Close outside [Low, High]', () => {
  const invalidBar: Bar[] = [
    { time: '2026-01-01', open: 100, high: 105, low: 99, close: 110, volume: 1000 }, // close > high
    { time: '2026-01-02', open: 102, high: 108, low: 101, close: 105, volume: 1200 },
  ];
...
Deno.test('validateMarketData: Large gap (>15%)', () => {
  const gapBars: Bar[] = [
    { time: '2026-01-01', open: 100, high: 105, low: 99, close: 100, volume: 1000 },
    { time: '2026-01-02', open: 120, high: 125, low: 119, close: 122, volume: 1200 }, // 20% gap
  ];
...
Deno.test('validatePositionConstraints: Quantity out of bounds (too large)', () => {
  ...
  const result = validatePositionConstraints(constraints);
  assertEquals(result.valid, false);
  assertEquals(result.errors.some(e => e.includes('Quantity out of bounds')), true);
});
...
Deno.test('validateSLTPBounds: TP <= SL', () => {
  ...
  assertEquals(result.valid, false);
  assertEquals(result.errors.some(e => e.includes('TP') && e.includes('SL')), true);
});
...
Deno.test('validateBeforeExecution: Multiple errors', () => {
  const invalidBars: Bar[] = [
    { time: '2026-01-01', open: null as any, high: 105, low: 99, close: 102, volume: 1000 },
    { time: '2026-01-02', open: 102, high: 108, low: 101, close: 105, volume: 1200 },
  ];
