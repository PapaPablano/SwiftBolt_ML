...
def add_files_to_xcode():
    ...
    if build_file_section:
        ...
        for bf in build_files:
            ...
            if any(
                ref["uuid"] == bf["file_ref"]
                and ref["type"] == "sourcecode.swift"
                for ref in file_refs
            ):
                ref_name = next(
                    ref["name"]
                    for ref in file_refs
                    if ref["uuid"] == bf["file_ref"]
                )
                new_bf = (
                    f'\t\t{bf["uuid"]} /* {ref_name} in Sources */ = '
                    '{isa = PBXBuildFile; fileRef = '
                    f'{bf["file_ref"]} /* {ref_name} */; }};\n'
                )
                new_build_files.append(new_bf)

        if new_build_files:
            insert_pos = build_file_section.end() - len(
    ...
    views_group = re.search(
        r'([A-F0-9]{24}) /\* Views \*/ = \{[^}]*children = \([^)]*\);',
        content,
    )
    if views_group:
        swift_ref = next(
            (
                ref
                for ref in file_refs
                if ref["name"] == "WebChartControlsView.swift"
            ),
            None,
        )
        if swift_ref:
            ...
            children_match = re.search(
                r'(children = \([^)]*)',
                content[views_group.start():views_group.end()],
            )
            if children_match:
                insert_text = (
                    "\n\t\t\t\t"
                    f"{swift_ref['uuid']} /* {swift_ref['name']} */,"
                )
                pos = views_group.start() + children_match.end()
                content = content[:pos] + insert_text + content[pos:]

    # Find WebChart group and add JS files
    webchart_group = re.search(
        r'([A-F0-9]{24}) /\* WebChart \*/ = \{[^}]*children = \([^)]*\);',
        content,
    )
    if webchart_group:
        js_refs = [
            ref
            for ref in file_refs
            if ref["type"] == "sourcecode.javascript"
        ]
        if js_refs:
            children_match = re.search(
                r'(children = \([^)]*)',
                content[webchart_group.start():webchart_group.end()],
            )
            if children_match:
                insert_texts = []
                for ref in js_refs:
    ...
    if sources_phase:
        ...
        if swift_build:
            files_match = re.search(
                r'(files = \([^)]*)',
                content[sources_phase.start():sources_phase.end()],
            )
            if files_match:
                ref_name = next(
                    ref["name"]
                    for ref in file_refs
                    if ref["uuid"] == swift_build["file_ref"]
                )
                insert_text = (
                    "\n\t\t\t\t"
                    f"{swift_build['uuid']} /* {ref_name} in Sources */,"
                )
...
if __name__ == "__main__":
    ...
    if add_files_to_xcode():
        print(
            "\n✅ Done! Rebuild your project in Xcode (Cmd+Shift+K, "
            "then Cmd+B)"
        )
    else:
