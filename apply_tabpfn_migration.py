...
Uses psycopg2 with DATABASE_URL (or ml config). Run from repo root:

    export DATABASE_URL="postgresql://..."   # if not in ml/.env
    python apply_tabpfn_migration.py

Or from ml directory:
    cd ml && python -c "exec(open('../apply_tabpfn_migration.py').read())"
"""
import os
import sys
from pathlib import Path

# Allow importing ml config
...
if not _database_url:
    print("=" * 80)
    print("DATABASE_URL not set. Cannot run migration without Postgres connection.")
    print("=" * 80)
    print("\nOption 1: Set connection string and re-run")
    print('  export DATABASE_URL="postgresql://user:pass@host:5432/postgres"')
    print("  python apply_tabpfn_migration.py")
    print("\nOption 2: Paste the SQL from supabase/migrations/20260131180000_add_model_type_to_ml_forecasts.sql")
    print("  into Supabase Dashboard → SQL Editor → New Query → Run")
    print("\nOption 3: Install psql and run: psql $DATABASE_URL -f supabase/migrations/20260131180000_add_model_type_to_ml_forecasts.sql")
    sys.exit(1)
...
def run():
    ...
    try:
        ...
        print("  Check DATABASE_URL and network (Supabase host must be reachable).")
        print("  Alternatively, run the SQL from supabase/migrations/20260131180000_add_model_type_to_ml_forecasts.sql in Supabase Dashboard → SQL Editor.")
        sys.exit(1)
    ...
    try:
        # 1. Add model_type column (same as migration DO block)
        ...
        # 5. View (matches migration: forecast_model_comparison, overall_label as direction)
    ...
    print("=" * 80)
    print("\nNext steps:")
    print("  1. Test TabPFN: cd ml && python -m src.unified_forecast_job --symbol AAPL --model-type tabpfn")
    print("  2. Compare models: python experiments/tabpfn_vs_xgboost.py --symbols AAPL,MSFT --generate")
