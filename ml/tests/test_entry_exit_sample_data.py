...
def create_iv_stats():
    ...
    return IVStatistics(
        iv_high=0.45, iv_low=0.15, iv_median=0.28, iv_current=0.25, days_of_data=252
    )
...
def print_contract_details(row, rank_type="ENTRY"):
    ...
    print(
        f"   IV: {row['iv']:.2%}, Delta: {row['delta']:.2f}, Volume: {row['volume']:.0f}, OI: {row['open_interest']:.0f}"
    )

    if rank_type == "ENTRY":
        print(f"\n   🎯 ENTRY RANK: {row.get('entry_rank', 0):.1f}/100")
        print(
            f"      └─ Entry Value:  {row.get('entry_value_score', 0):.1f} × 40% = {row.get('entry_value_score', 0)*0.40:.1f}"
        )
        print(
            f"      └─ Catalyst:     {row.get('catalyst_score', 0):.1f} × 35% = {row.get('catalyst_score', 0)*0.35:.1f}"
        )
        print(
            f"      └─ Greeks:       {row.get('greeks_score', 0):.1f} × 25% = {row.get('greeks_score', 0)*0.25:.1f}"
        )
    elif rank_type == "EXIT":
        print(f"\n   🚪 EXIT RANK: {row.get('exit_rank', 0):.1f}/100")
        print(
            f"      └─ Profit:       {row.get('profit_protection_score', 0):.1f} × 50% = {row.get('profit_protection_score', 0)*0.50:.1f}"
        )
        print(
            f"      └─ Deterioration: {row.get('deterioration_score', 0):.1f} × 30% = {row.get('deterioration_score', 0)*0.30:.1f}"
        )
        print(
            f"      └─ Time Urgency: {row.get('time_urgency_score', 0):.1f} × 20% = {row.get('time_urgency_score', 0)*0.20:.1f}"
        )
    else:  # MONITOR
        print(f"\n   📊 MONITOR RANK: {row.get('composite_rank', 0):.1f}/100")
        print(
            f"      └─ Momentum:     {row.get('momentum_score', 0):.1f} × 40% = {row.get('momentum_score', 0)*0.40:.1f}"
        )
        print(
            f"      └─ Value:        {row.get('value_score', 0):.1f} × 35% = {row.get('value_score', 0)*0.35:.1f}"
        )
        print(
            f"      └─ Greeks:       {row.get('greeks_score', 0):.1f} × 25% = {row.get('greeks_score', 0)*0.25:.1f}"
        )
...
def test_entry_mode():
    ...
    results = ranker.rank_options(
        chain, mode=RankingMode.ENTRY, iv_stats=iv_stats, options_history=history
    )
    ...
    if top.get("entry_value_score", 0) > 75:
        print(
            f"   ✓ CHEAP: Entry value score {top.get('entry_value_score', 0):.0f} indicates low IV"
        )
    if top.get("catalyst_score", 0) > 70:
        print(
            f"   ✓ CATALYST: Catalyst score {top.get('catalyst_score', 0):.0f} shows strong momentum"
        )
    if top.get("greeks_score", 0) > 70:
        print(
            f"   ✓ POSITIONED: Greeks score {top.get('greeks_score', 0):.0f} means good delta/gamma"
        )
...
def test_exit_mode():
    ...
    results = ranker.rank_options(
        chain, mode=RankingMode.EXIT, options_history=history, entry_data=entry_data
    )
    ...
    for i, (idx, row) in enumerate(results_sorted.head(3).iterrows(), 1):
        ...
        print(
            f"   💰 P&L: {pnl_pct:+.1f}% (${row['mark']:.2f} from ${entry_data['entry_price']:.2f})"
        )
    ...
    if top.get("deterioration_score", 0) > 60:
        print(
            f"   ✓ FADING: Deterioration score {top.get('deterioration_score', 0):.0f} shows momentum decay"
        )
    if top.get("time_urgency_score", 0) > 60:
        print(
            f"   ✓ TIME: Urgency score {top.get('time_urgency_score', 0):.0f} indicates approaching expiration"
        )
...
def test_monitor_mode():
    ...
    results = ranker.rank_options(
        chain, mode=RankingMode.MONITOR, iv_stats=iv_stats, options_history=history
    )
...
def compare_modes():
    ...
    entry_results = ranker.rank_options(
        chain, mode=RankingMode.ENTRY, iv_stats=iv_stats, options_history=history
    )
    exit_results = ranker.rank_options(
        chain, mode=RankingMode.EXIT, options_history=history, entry_data={"entry_price": 1.80}
    )
    monitor_results = ranker.rank_options(
        chain, mode=RankingMode.MONITOR, iv_stats=iv_stats, options_history=history
    )
    ...
    print(
        f"      └─ Value {entry_row.get('entry_value_score', 0):.0f} + Catalyst {entry_row.get('catalyst_score', 0):.0f} + Greeks {entry_row.get('greeks_score', 0):.0f}"
    )

    print(f"\n   EXIT MODE:    {exit_row['exit_rank']:.1f}/100")
    print(
        f"      └─ Profit {exit_row.get('profit_protection_score', 0):.0f} + Deterioration {exit_row.get('deterioration_score', 0):.0f} + Time {exit_row.get('time_urgency_score', 0):.0f}"
    )

    print(f"\n   MONITOR MODE: {monitor_row['composite_rank']:.1f}/100")
    print(
        f"      └─ Momentum {monitor_row.get('momentum_score', 0):.0f} + Value {monitor_row.get('value_score', 0):.0f} + Greeks {monitor_row.get('greeks_score', 0):.0f}"
    )
    ...
    if entry_row["entry_rank"] > monitor_row["composite_rank"]:
        print(
            f"   Entry ({entry_row['entry_rank']:.0f}) > Monitor ({monitor_row['composite_rank']:.0f})"
        )
...
def run_all_tests():
    ...
    try:
        ...
        entry_has_nan = (
            entry_results["entry_rank"].isna().any() or np.isinf(entry_results["entry_rank"]).any()
        )
        exit_has_nan = (
            exit_results["exit_rank"].isna().any() or np.isinf(exit_results["exit_rank"]).any()
        )
        monitor_has_nan = (
            monitor_results["composite_rank"].isna().any()
            or np.isinf(monitor_results["composite_rank"]).any()
        )
