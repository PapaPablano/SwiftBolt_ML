#!/usr/bin/env python3
"""
Market Regime-Aware Testing for SwiftBolt ML
Tests stocks across different market regimes with proper data handling
...
REGIMES = {
    'crash_2022': {
        ...
        'threshold_override': 0.02,  # wider move threshold in crash
    },
    'recovery_2023': {
        'start': '2022-11-01',
        'end': '2023-12-31',
    ...
    'bull_2024': {
        ...
        'threshold_override': 0.012,  # tighter in low-vol bull
    },
    'rotation_2022': {
        'start': '2021-12-01',
...
def evaluate_stock_in_regime(
    ...
    if len(df) < 50:
        logger.warning(f"    ⚠️  Insufficient data: {len(df)} bars (need 50+)")
        return None
    ...
    try:
        ...
        # Walk-forward validation with per-window NaN handling (no leakage)
        min_train_size = WF_MIN_TRAIN_SIZE
        test_window = WF_TEST_WINDOW
        embargo = WF_EMBARGO_BARS
        # After train_end we leave embargo bars, then test_window bars
...
def run_regime_tests():
    ...
    print("="*100)
    print("MARKET REGIME-AWARE BACKTESTING")
...
def generate_summary_report(results: dict):
    ...
    print(f"Average accuracy: {df_summary['Accuracy'].mean():.1%}")
    print(f"Best result: {df_summary['Accuracy'].max():.1%}")
    print(f"Worst result: {df_summary['Accuracy'].min():.1%}")
    print(f"Std deviation: {df_summary['Accuracy'].std():.1%}")

    # Accuracy distribution
    print(f"\nAccuracy distribution:")
    print(f"  >55%: {(df_summary['Accuracy'] > 0.55).sum()} tests")
    ...
    # Save to CSV
    output_path = 'regime_test_results.csv'
    ...
    for category, config in STOCKS.items():
        ...
        for symbol in config['symbols']:
            ...
            for regime_name, regime in REGIMES.items():
                ...
                print(f"    {regime['type']:20s}: {len(df_regime):3d} bars")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Market Regime-Aware Testing')
