#!/usr/bin/env python3
"""
Market Regime-Aware Testing for SwiftBolt ML - IMPROVED VERSION (BALANCED)
Tests stocks across different market regimes with enhanced validation

IMPROVEMENTS:
- Extended Growth Rotation period (2021-09-01 vs 2021-12-01) for more data
- Balanced walk-forward validation parameters (55 train, 7 test, 1 embargo)
- Regime-specific threshold overrides
- Moderate minimum requirements for reliable yet comprehensive results

Usage (from ml/):
    cd /Users/ericpeterson/SwiftBolt_ML/ml
    python test_regimes_IMPROVED.py
    python test_regimes_IMPROVED.py --test-data
...
def get_regime_requirements(regime_name: str) -> dict:
    """Get minimum requirements per regime (lower for Growth Rotation)."""
...
def evaluate_stock_in_regime(
    ...
    try:
        ...
        if X is None or y is None or len(X) < 30:
            logger.warning(f"    ⚠️  Insufficient features: {len(X) if X is not None else 0} samples (need 30+)")
            return None
...
def run_regime_tests():
    ...
    print("Testing 10 stocks across 4 market regimes with balanced validation")
    print("Parameters: regime-specific (rotation_2022: train=40, test=5, min_bars=50, min_windows=3; others: 55/7/75/4)")
    print("="*100)
...
def generate_summary_report(results: dict):
    ...
    # Save to CSV
    output_path = 'regime_test_results_improved.csv'
    ...
    for category, config in STOCKS.items():
        ...
        for symbol in config['symbols']:
            ...
            for regime_name, regime in REGIMES.items():
                ...
                meets_min = "✅" if len(df_regime) >= 75 else "⚠️"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Market Regime-Aware Testing (Improved)')
