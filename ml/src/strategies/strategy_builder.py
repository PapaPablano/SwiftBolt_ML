...
class Strategy:
    ...
    def plot(self, **kwargs):
        ...
        return diagram.plot(
            current_price=self.underlying_price if self.underlying_price > 0 else None, **kwargs
        )
...
class StrategyBuilder:
    ...
    @staticmethod
    def long_call(
        underlying_price: float, strike: float, premium: float, quantity: int = 1
    ) -> Strategy:
    ...
    @staticmethod
    def custom_strategy(
        name: str, legs: List[Dict], underlying_price: float, description: str = ""
    ) -> Strategy:
        ...
        Returns:
            Strategy object
        """
        strategy = Strategy(name=name, underlying_price=underlying_price, description=description)
        ...
        total_cost = sum(
            leg.premium * abs(leg.quantity) * 100 for leg in strategy.legs if leg.quantity > 0
        )
...
if __name__ == "__main__":
    ...
    strategy3 = builder.straddle(
        underlying_price=underlying, strike=100, call_premium=5.0, put_premium=5.0, long=True
    )
