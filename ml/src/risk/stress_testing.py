...
HISTORICAL_SCENARIOS = {
    ...
    "1987_black_monday": {
        ...
        "shocks": {"SPY": -0.22, "QQQ": -0.25, "TLT": 0.10, "VIX": 2.0},  # Single day
    },
...
class StressTester:
    ...
    def __init__(
        self, positions: Dict[str, float], current_prices: Dict[str, float], var_level: float = 0.05
    ):
        ...
        self.portfolio_value = sum(
            positions.get(symbol, 0) * current_prices.get(symbol, 0) for symbol in positions.keys()
        )
    ...
    def _apply_shocks(self, shocks: Dict[str, float]) -> Dict[str, float]:
    ...
    def historical_stress_test(self, scenario_name: str) -> StressTestResult:
        """Run historical stress test.
        ...
        if scenario_name not in HISTORICAL_SCENARIOS:
            ...
            raise ValueError(f"Unknown scenario: {scenario_name}. " f"Available: {available}")
        ...
        portfolio_change_pct = (
            portfolio_change / self.portfolio_value if self.portfolio_value > 0 else 0
        )
    ...
    def custom_scenario(
        self, shocks: Dict[str, float], scenario_name: str = "Custom Scenario"
    ) -> StressTestResult:
    ...
    def correlation_breakdown(self, correlation_increase: float = 0.50) -> StressTestResult:
...
if __name__ == "__main__":
    ...
    current_prices = {"SPY": 450, "QQQ": 380, "TLT": 95, "GLD": 180}

    # Calculate portfolio value
    portfolio_value = sum(positions[s] * current_prices[s] for s in positions.keys())
    ...
    custom_shocks = {"SPY": -0.10, "QQQ": -0.15, "TLT": 0.05, "GLD": 0.02}
    ...
    for result in all_results:
        print(
            f"{result.scenario_name:<40} {result.portfolio_change_pct*100:<15.2f}% {result.severity:<10}"
        )
