...
class RiskValidator:
    ...
    def validate_trade(self, trade: Dict, current_portfolio: Dict, verbose: bool = True) -> Dict:
        ...
        if current_portfolio.get("total_margin", 0) > 0:
            current_usage = (
                current_portfolio.get("margin_used", 0) / current_portfolio["total_margin"]
            )
            trade_margin = trade.get("margin_required", 0)
            new_usage = (current_portfolio["margin_used"] + trade_margin) / current_portfolio[
                "total_margin"
            ]
    ...
    def check_portfolio_health(self, portfolio: Dict, verbose: bool = True) -> Dict:
