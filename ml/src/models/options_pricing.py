...
class BlackScholesModel:
    ...
    def calculate_greeks(
        ...
        if T <= 0:
            ...
            return OptionsPricing(
                theoretical_price=price, delta=delta, gamma=0.0, theta=0.0, vega=0.0, rho=0.0
            )
        ...
        return OptionsPricing(
            theoretical_price=price, delta=delta, gamma=gamma, theta=theta, vega=vega, rho=rho
        )
    ...
    def verify_put_call_parity(
        self, S: float, K: float, T: float, sigma: float, tolerance: float = 0.01
    ) -> bool:
