...
class IsotonicCalibrator:
    ...
    def fit(
        ...
        if n_samples < self.n_bins * self.min_samples_per_bin:
            logger.warning(
                f"Insufficient samples ({n_samples}) for {self.n_bins} bins. " f"Reducing bins."
            )
        ...
        bin_return_percentiles = self._returns_to_percentiles(bin_return_means, forward_returns)

        # Check monotonicity
        is_monotonic = np.all(np.diff(bin_return_percentiles) >= 0) and np.all(
            np.diff(bin_positive_probs) >= 0
        )
        ...
        extended_percentiles = np.concatenate(
            [[bin_return_percentiles[0]], bin_return_percentiles, [bin_return_percentiles[-1]]]
        )
        extended_probs = np.concatenate(
            [[bin_positive_probs[0]], bin_positive_probs, [bin_positive_probs[-1]]]
        )
    ...
    def calibrate_rankings(
        ...
        df["calibrated_rank"] = (
            df["calibrated_return_pct"] * 0.6 + df["calibrated_positive_prob"] * 100 * 0.4
        )
    ...
    def load(cls, path: str) -> "IsotonicCalibrator":
        ...
        if state.get("is_fitted"):
            ...
            extended_centers = np.concatenate([[0], calibrator._bin_centers, [100]])
            extended_percentiles = np.concatenate(
                [
