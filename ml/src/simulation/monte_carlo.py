    ...
    def geometric_brownian_motion_with_jumps(
        ...
        for path_idx in range(n_paths):
            if n_jumps[path_idx] > 0:
                ...
                jump_sizes = np.exp(jump_mean + jump_std * np.random.randn(n_jumps[path_idx]))
...
class MonteCarloSimulator:
    ...
    def price_european_option(self, K: float, option_type: str = "call") -> Dict[str, float]:
    ...
    def calculate_greeks(
        self, K: float, option_type: str = "call", epsilon: float = 0.01
    ) -> Dict[str, float]:
    ...
    def calculate_var(
        self, confidence_level: float = 0.95, holding_period_days: int = 1
    ) -> Dict[str, float]:
...
if __name__ == "__main__":
    ...
    sim = MonteCarloSimulator(S0=S0, r=r, sigma=sigma, T=T, n_simulations=10000, seed=42)
    ...
    print(
        f"  95% CI: ${call_result['confidence_interval'][0]:.4f} - "
    ...
    try:
        ...
        print(f"\nComparison (Call @ K=${K}):")
        print(
            f"  Price: MC=${call_result['price']:.4f}, BS=${bs_pricing.theoretical_price:.4f}, "
            f"Diff={abs(call_result['price'] - bs_pricing.theoretical_price):.4f}"
        )
        print(
            f"  Delta: MC={greeks['delta']:.4f}, BS={bs_pricing.delta:.4f}, "
