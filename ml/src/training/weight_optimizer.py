...
class EnsembleWeightOptimizer:
    ...
    def optimize_weights(
        ...
        y_target = np.array([label_to_value.get(label, 0) for label in actual_labels])
        ...
        for i, model_name in enumerate(model_names):
            ...
            X_ensemble[:, i] = [label_to_value.get(pred, 0) for pred in preds]
        ...
        self.weights = {
            name: float(weight) for name, weight in zip(model_names, normalized_weights)
        }
