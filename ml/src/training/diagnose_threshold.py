...
def plot_distribution(
    ...
    ax1.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Current ({current_threshold*100:.1f}%)",
    )
    ...
    ax1.axvline(
        ...
        linestyle="-",
        linewidth=2,
        label=f"Recommended ({recommended_threshold*100:.1f}%)",
    )
...
def main():
    ...
    parser.add_argument(
        "--bars", type=int, default=500, help="Number of bars to analyze (default: 500)"
    )
    parser.add_argument(
        "--current-threshold", type=float, default=0.015, help="Current threshold (default: 0.015)"
    )
    parser.add_argument("--plot", action="store_true", help="Generate plots")
    parser.add_argument(
        "--target-neutral", type=float, default=40.0, help="Target neutral percentage (default: 40)"
    )
    ...
    print(
        f"BEARISH:  {current_stats['bearish_count']:4d} samples ({current_stats['bearish_pct']:.1f}%)"
    )
    print(
        f"NEUTRAL:  {current_stats['neutral_count']:4d} samples ({current_stats['neutral_pct']:.1f}%)"
    )
    print(
        f"BULLISH:  {current_stats['bullish_count']:4d} samples ({current_stats['bullish_pct']:.1f}%)"
    )
    ...
    print(
        f"{'Threshold':>10} | {'Bearish':>8} | {'Neutral':>8} | {'Bullish':>8} | {'Non-Neutral':>12} | {'Balance':>8}"
    )
    print("-" * 80)
