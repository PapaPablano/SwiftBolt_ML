...
creating a comprehensive predictive portrait from near-term precision to long-term context.
"""
...
class MultiHorizonForecast:
    """
    Multi-horizon forecast for a specific timeframe.
    
    Contains forecasts for multiple horizons (e.g., M15 has 4h, 1d, 1w forecasts)
    with handoff confidence metrics for seamless transitions between timeframes.
    """
...
def calculate_handoff_confidence(
    ...
    elif (current_forecast.direction == "NEUTRAL" or 
          next_timeframe_forecast.direction == "NEUTRAL"):
    ...
    if current_forecast.target > 0 and next_timeframe_forecast.target > 0:
        target_diff_pct = abs(
            current_forecast.target - next_timeframe_forecast.target
...
def build_cascading_consensus(
    ...
    """
    Build consensus forecast for a specific horizon from multiple timeframes.
    
    Combines overlapping forecasts (e.g., H4 30d + D1 30d) using confidence weighting.
    ...
    # Calculate effective weights (timeframe weight × forecast confidence × handoff)
    effective_weights = {}
    total_weight = 0.0
    ...
    for timeframe, forecast in horizon_forecasts.items():
        ...
        # Get timeframe weight (shorter timeframes get more weight for near-term)
