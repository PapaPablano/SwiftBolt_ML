...
def validate_against_tradingview(
    ...
    if 'supertrend' in merged.columns and 'supertrend_stop' in merged.columns:
        ...
        if valid_idx.sum() > 0:
            error = abs(merged.loc[valid_idx, 'supertrend'] - 
                       merged.loc[valid_idx, 'supertrend_stop']).mean()
    ...
    for indicator in ['k', 'd', 'j']:
        ...
        if our_col in merged.columns and tv_col in merged.columns:
            ...
            if valid_idx.sum() > 0:
                error = abs(merged.loc[valid_idx, our_col] - 
                           merged.loc[valid_idx, tv_col]).mean()
    ...
    if 'adx' in merged.columns and 'adx_current' in merged.columns:
        ...
        if valid_idx.sum() > 0:
            error = abs(merged.loc[valid_idx, 'adx'] - 
                       merged.loc[valid_idx, 'adx_current']).mean()
