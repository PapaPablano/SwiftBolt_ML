...
class VolatilityAnalyzer:
    ...
    def calculate_historical_volatility(
        prices: pd.Series, window: int = 20, annualization_factor: int = 252
    ) -> float:
    ...
    def calculate_iv_rank(
        current_iv: float, iv_history: pd.Series, lookback_days: int = 252
    ) -> float:
    ...
    def calculate_expected_move(
        stock_price: float, implied_vol: float, days_to_expiration: int
    ) -> Dict[str, float]:
