...
class WalkForwardOptimizer:
    ...
    def _test_window(
        self, data: pd.DataFrame, start_date: datetime, end_date: datetime, params: Dict[str, Any]
    ) -> Dict[str, float]:
    ...
    def run(self, data: pd.DataFrame) -> WalkForwardResults:
        ...
        while current_date < end_date:
            ...
            logger.info(
                f"Window {window_id}: IS {is_start.date()} to {is_end.date()}, "
    ...
    def _calculate_aggregate_stats(self, results: WalkForwardResults):
        ...
        for param_name in self.param_grid.keys():
            param_values = [w.best_params.get(param_name, 0) for w in results.window_results]
...
if __name__ == "__main__":
    ...
    data = pd.DataFrame({"price": prices, "returns": returns}, index=dates)
    ...
    def momentum_strategy(
        data: pd.DataFrame, lookback: int = 20, threshold: float = 0.0
    ) -> Dict[str, float]:
        ...
        return {"sharpe_ratio": sharpe, "total_return": total_return, "volatility": volatility}
    ...
    wfo = WalkForwardOptimizer(
        ...
        param_grid={"lookback": [10, 20, 30], "threshold": [0.0, 0.01, 0.02]},
        is_period_days=252,  # 1 year training
        oos_period_days=63,  # 3 months testing
        metric="sharpe_ratio",
    ...
    for w in results.window_results:
        ...
        params_str = (
            f"LB={w.best_params.get('lookback', 'N/A')}, TH={w.best_params.get('threshold', 'N/A')}"
        )
