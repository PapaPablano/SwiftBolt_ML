...
class PortfolioOptimizer:
    ...
    def max_sharpe_portfolio(self, constraints: Optional[Dict] = None) -> PortfolioAllocation:
        ...
        if "target_return" in constraints:
            ...
            cons.append({"type": "eq", "fun": lambda w: np.sum(self.mean_returns * w) - target})
    ...
    def min_variance_portfolio(self, constraints: Optional[Dict] = None) -> PortfolioAllocation:
    ...
    def efficient_portfolio(
        self, target_return: float, constraints: Optional[Dict] = None
    ) -> PortfolioAllocation:
    ...
    def risk_parity_portfolio(self) -> PortfolioAllocation:
        ...
        # Optimize
        result = minimize(
            risk_parity_objective, w0, method="SLSQP", bounds=bounds, constraints=cons
        )
...
if __name__ == "__main__":
    ...
    # Correlation matrix
    corr = np.array(
        [[1.0, 0.7, 0.5, 0.3], [0.7, 1.0, 0.6, 0.4], [0.5, 0.6, 1.0, 0.5], [0.3, 0.4, 0.5, 1.0]]
    )
    ...
    returns_df = pd.DataFrame(returns, columns=["Stock A", "Stock B", "Stock C", "Bond"])
    ...
    for name, portfolio in strategies:
        print(
            f"{name:<20} {portfolio.expected_return*100:<12.2f}% "
