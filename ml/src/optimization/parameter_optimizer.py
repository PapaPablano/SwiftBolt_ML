...
class ParameterOptimizer:
    ...
    def _evaluate_params(self, data: pd.DataFrame, params: Dict[str, Any]) -> Optional[float]:
    ...
    def optimize(
        self, data: pd.DataFrame, method: str = "grid", n_trials: int = 100, verbose: bool = True
    ) -> OptimizationResult:
        """Optimize parameters.
        ...
        for i, params in enumerate(param_combinations):
            ...
            if verbose and (i + 1) % max(1, total // 10) == 0:
                logger.info(
                    f"Progress: {i+1}/{total} ({100*(i+1)/total:.0f}%), "
...
if __name__ == "__main__":
    ...
    def test_strategy(data: pd.DataFrame, ma_fast: int, ma_slow: int, threshold: float) -> Dict:
        ...
        # Generate signals
        data["signal"] = np.where((data["ma_fast"] > data["ma_slow"] * (1 + threshold)), 1, 0)
