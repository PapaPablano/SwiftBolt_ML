...
class CostOptimizer:
    ...
    def __init__(
        self, commission_rate: float = 0.001, spread_bps: float = 5.0, impact_coef: float = 0.1
    ):
    ...
    def optimize_rebalancing(
        ...
        if result.success:
            ...
            logger.info(
                f"Optimization successful: cost reduced by {(objective(x0) - result.fun) / objective(x0):.2%}"
            )
