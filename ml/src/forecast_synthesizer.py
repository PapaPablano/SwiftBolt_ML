...
class ForecastSynthesizer:
    ...
    def generate_1d_forecast(
        ...
        support_weighted = self._calculate_sr_weighted(sr_response, is_support=True, current_price=current_price)
        resistance_weighted = self._calculate_sr_weighted(sr_response, is_support=False, current_price=current_price)
        ...
        poly_target_resistance = poly_forecast_resistance[0] if poly_forecast_resistance else current_price
        ...
        logistic_resistance_prob = (
            ...
            if logistic_resistance_levels else 0.5
        )
        logistic_support_prob = (
            logistic_support_levels[0].get('probability', 0.5)
            if logistic_support_levels else 0.5
        )
        ...
        return ForecastResult(
            ...
            ml_component=round(current_price * (1 + 0.02 * ml_bias), 2) if ml_bias != 0 else round(current_price, 2),
    ...
    def generate_1w_forecast(
        ...
        target_1w = (
            ...
            (daily_forecast.upper_band if primary_direction == "BULLISH" else daily_forecast.lower_band) * 0.25
        )
    ...
    def _build_reasoning(
        ...
        reasoning = f"{direction} forecast: {', '.join(drivers)}" if drivers else f"{direction} signal with low conviction"
