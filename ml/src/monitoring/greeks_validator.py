...
class GreeksValidator:
    ...
    def _validate_single_option(self, row: pd.Series):
        ...
        try:
            ...
            # Check each Greek
            self._check_greek(
                row, ticker, "delta", row["delta"], theoretical.delta, self.DELTA_TOLERANCE
            )
            self._check_greek(
                row, ticker, "gamma", row["gamma"], theoretical.gamma, self.GAMMA_TOLERANCE
            )
            self._check_greek(
                row, ticker, "theta", row["theta"], theoretical.theta, self.THETA_TOLERANCE
            )
            self._check_greek(
                row, ticker, "vega", row["vega"], theoretical.vega, self.VEGA_TOLERANCE
            )
...
def main():
    ...
    parser.add_argument(
        "--min-volume", type=int, default=50, help="Minimum daily volume (default: 50)"
    )

    parser.add_argument("--save", action="store_true", help="Save discrepancies to CSV file")
    ...
    # Validate
    discrepancies = validator.validate_options_greeks(
        symbols=args.symbols, min_volume=args.min_volume
    )
