...
class OptionsPricePredictor:
    ...
    def train(
        ...
        logger.info(
            f"Training complete. Val R²: {metrics['val_r2']:.4f}, Val RMSE: {metrics['val_rmse']:.4f}"
        )
    ...
    def predict_with_confidence(
        ...
        confidence = (
            1 - (interval_width / max_width) if max_width > 0 else np.ones_like(predictions)
        )

        return PredictionResult(
            predictions=predictions, lower_bound=lower, upper_bound=upper, confidence=confidence
        )

    def _ensemble_confidence(
        self, X: pd.DataFrame, confidence_level: float
    ...
    def get_feature_importance(self) -> Optional[pd.Series]:
        ...
        if hasattr(self.model, "feature_importances_"):
            return pd.Series(
                self.model.feature_importances_, index=range(len(self.model.feature_importances_))
            ).sort_values(ascending=False)

        elif hasattr(self.model, "coef_"):
            return pd.Series(abs(self.model.coef_), index=range(len(self.model.coef_))).sort_values(
                ascending=False
            )
    ...
    def load(cls, filepath: str) -> "OptionsPricePredictor":
        ...
        predictor = cls(model_type=model_data["model_type"], **model_data["model_params"])
...
if __name__ == "__main__":
    ...
    if not SKLEARN_AVAILABLE:
        ...
        for model_type in ["linear", "random_forest", "gradient_boosting"]:
            ...
            print(
                f"  Confidence interval: [{result.lower_bound[0]:.4f}, {result.upper_bound[0]:.4f}]"
            )
