...
class ModelTrainer:
    ...
    def __init__(
        self, config: TrainingConfig, use_wandb: bool = False, wandb_project: str = "swiftbolt-ml"
    ):
        """Initialize trainer.
        ...
        logger.info(
            f"ModelTrainer initialized: {config.model_type}, horizon={config.target_horizon}d"
        )
    ...
    def train(
        ...
        if self.use_wandb:
            wandb.init(project=self.wandb_project, name=run_name, config=self.config.__dict__)

        try:
            # Create features
            logger.info("Creating features...")
            features = self.feature_engineer.create_features(
                price_data, options_data, include_greeks=self.config.include_greeks
            )
            ...
            logger.info(
                f"Training on {len(features_clean)} samples with {len(features_clean.columns)} features"
            )

            # Split data (chronological)
            train_size = int(
                len(features_clean) * (1 - self.config.test_split - self.config.validation_split)
            )
            ...
            self.model = OptionsPricePredictor(
                model_type=self.config.model_type, **self.config.model_params
            )
