...
class ValidationService:
    ...
    async def _get_backtesting_score(self, symbol: str) -> float:
        ...
        try:
            ...
            # First get symbol_id
            symbol_result = (
                self.db.client.table("symbols").select("id").eq("ticker", symbol).execute()
            )
    ...
    async def _get_live_score(self, symbol: str) -> float:
        ...
        try:
            ...
            if not result.data or len(result.data) < 5:
                logger.warning(
                    f"Insufficient live data for {symbol} (got {len(result.data or [])} predictions)"
                )
    ...
    async def _get_multi_tf_scores(self, symbol: str) -> Dict[str, float]:
        ...
        try:
            ...
            tf_map = {"M15": "m15", "H1": "h1", "H4": "h4", "D1": "d1", "W1": "w1"}

            # Map signal to score
            signal_to_score = {"BULLISH": 1.0, "BEARISH": -1.0, "NEUTRAL": 0.0}
            ...
            for tf in timeframes:
                try:
                    ...
                    if result.data and len(result.data) > 0:
                        ...
                        logger.debug(
                            f"{symbol} {tf} score: {score:.2f} (signal={signal}, accuracy={accuracy:.2f})"
                        )
                    else:
    ...
    async def _store_validation_result(self, symbol: str, result: UnifiedPrediction) -> None:
        """
