...
class PayoffCalculator:
    ...
    @staticmethod
    def calculate_payoff(legs: List[OptionLeg], underlying_prices: np.ndarray) -> np.ndarray:
        ...
        for leg in legs:
            leg_payoff = np.array([leg.payoff_at_price(price) for price in underlying_prices])
    ...
    @staticmethod
    def find_break_even(
        legs: List[OptionLeg], price_range: Tuple[float, float] = (0, 500), precision: float = 0.01
    ) -> List[float]:
        ...
        for i in range(len(payoffs) - 1):
            if payoffs[i] * payoffs[i + 1] < 0:  # Sign change
                ...
                be = prices[i] - payoffs[i] * (prices[i + 1] - prices[i]) / (
    ...
    @staticmethod
    def max_profit(
        legs: List[OptionLeg], price_range: Tuple[float, float] = (0, 500)
    ) -> Tuple[float, float]:
...
class PayoffDiagram:
    ...
    def plot(
        ...
        fig.update_layout(
            title=title
            or f"{self.strategy_name}<br><sub>{cost_type}: ${abs(initial_cost):.2f}</sub>",
    ...
    def get_summary(self) -> Dict:
        ...
        max_strike = max(strikes)
        range_width = max_strike - min_strike
        price_range = (max(0, min_strike - range_width * 0.5), max_strike + range_width * 0.5)
