...
class StatisticalSignificanceTester:
    ...
    def bootstrap_confidence_intervals(
        ...
        for _ in range(n_bootstrap):
            ...
            # Calculate metrics
            if "MAE" in metrics:
                bootstrap_results["MAE"].append(
                    mean_absolute_error(y_true_boot, y_pred_boot)
                )
            if "RMSE" in metrics:
            ...
            if "R2" in metrics:
                bootstrap_results["R2"].append(
                    r2_score(y_true_boot, y_pred_boot)
                )
            if "MAPE" in metrics:
    ...
    def generate_full_report(
        ...
        report = {
            ...
            "n_samples": len(y_true),
            "confidence_level": self.confidence_level,
            "confidence_intervals": {ci.metric: {
