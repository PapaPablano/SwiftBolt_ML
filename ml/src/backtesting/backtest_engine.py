...
class BacktestEngine:
    ...
    def load_historical_data(
        self, ohlc_data: pd.DataFrame, options_data: Optional[pd.DataFrame] = None
    ):
    ...
    def get_current_prices(self, date: pd.Timestamp) -> Dict[str, float]:
        ...
        if self.ohlc_data is not None:
            ...
            date_naive = (
                date.tz_localize(None) if hasattr(date, "tz") and date.tz is not None else date
            )
            ...
            for _, opt in options_today.iterrows():
                ...
                if "bid" in opt and "ask" in opt:
                    ...
                    pricing = self.bs_model.calculate_greeks(
                        S=underlying_price, K=strike, T=dte / 365, sigma=iv, option_type=option_type
                    )
