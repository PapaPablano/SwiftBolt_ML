...
class Trade:
    ...
    def total_cost(self) -> float:
        ...
        is_stock = (
            symbol_upper == "STOCK"
            or (
                len(self.symbol) <= 6 and self.symbol.isalpha()
            )  # Short alphabetic symbols like "AAPL", "TSLA"
            or (
                hasattr(self, "underlying_price")
                and self.underlying_price > 0
                and abs(self.price - self.underlying_price) / self.underlying_price < 0.1
            )  # Price close to underlying (within 10%)
        )

        is_option = not is_stock and (
            len(self.symbol) > 10  # Options symbols are typically longer
            or (
                hasattr(self, "underlying_price")
                and self.underlying_price > 0
                and self.price < self.underlying_price * 0.3
            )  # Options are much cheaper than underlying
            or "_" in self.symbol  # Options often have underscores
            or (
                "C" in self.symbol[-15:] and any(c.isdigit() for c in self.symbol[-15:])
            )  # Has Call indicator with digits
            or (
                "P" in self.symbol[-15:] and any(c.isdigit() for c in self.symbol[-15:])
            )  # Has Put indicator with digits
        )
...
class TradeLogger:
    ...
    def __init__(self):
        ...
        self._pnl_by_trade_id: Dict[str, float] = (
            {}
        )  # trade_id -> realized P&L for SELLs that closed
    ...
    def log_trade(self, trade: Trade):
        ...
        logger.debug(
            f"Logged trade: {trade.action} {trade.quantity} {trade.symbol} @ ${trade.price:.2f}"
        )
