...
class GeneticOptimizer:
    ...
    def __init__(self, config: OptimizationConfig):
        ...
        logger.info(
            f"GeneticOptimizer initialized: pop={config.population_size}, gen={config.generations}"
        )
    ...
    def optimize(self, fitness_func: Callable, gene_bounds: List[tuple]) -> tuple:
        ...
        for generation in range(self.config.generations):
            ...
            if (generation + 1) % 10 == 0:
                logger.info(
                    f"Gen {generation+1}/{self.config.generations}: Best fitness={self.best_fitness:.4f}"
                )
    ...
    def _mutate(self, individual, gene_bounds):
        ...
        individual[gene_idx] += (
            np.random.randn() * 0.1 * (gene_bounds[gene_idx][1] - gene_bounds[gene_idx][0])
        )
        individual[gene_idx] = np.clip(
            individual[gene_idx], gene_bounds[gene_idx][0], gene_bounds[gene_idx][1]
        )
        return individual
...
if __name__ == "__main__":
    ...
    def fitness(individual):
        ...
        )  # Negative because we minimize distance from 0
