"""
Backfill Alpaca daily (d1) bars into ohlc_bars_v2 for regime/walk-forward use.

Usage:
    cd ml && python scripts/backfill_ohlc_d1_alpaca.py
...
# Symbols used by pipeline_audit + test_regimes_fixed (regime coverage)
DEFAULT_SYMBOLS = [
    "AAPL", "MSFT", "NVDA", "PG", "KO",  # pipeline_audit
    "JNJ", "MRK", "AMGN", "BRK.B", "MU", "ALB",  # test_regimes_fixed
]
...
def fetch_alpaca_d1_bars(
    symbol: str,
    start_date: datetime,
    end_date: datetime,
) -> list[dict]:
    """Fetch daily (1Day) bars from Alpaca. Returns list of {ts, open, high, low, close, volume}."""
    ...
    while page_count < max_pages:
        ...
        if not bars_data and page_count == 0:
            logger.warning("No d1 data for %s", symbol)
            return []
    ...
    logger.info("Fetched %s d1 bars for %s", len(all_bars), symbol)
    return all_bars


def persist_d1_v2(db: SupabaseDatabase, symbol: str, bars: list[dict]) -> int:
    """Upsert daily bars into ohlc_bars_v2 (provider=alpaca, timeframe=d1). Returns count persisted."""
    if not bars:
        return 0
    ...
    try:
        ...
        for i in range(0, len(batch), batch_size):
            ...
            inserted += len(chunk)
        logger.info("Persisted %s d1 bars for %s to ohlc_bars_v2", inserted, symbol)
        return inserted
    except Exception as e:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backfill Alpaca daily bars into ohlc_bars_v2")
    ...
    for symbol in args.symbols:
        bars = fetch_alpaca_d1_bars(symbol, start_date, end_date)
        if bars:
            total += persist_d1_v2(db, symbol, bars)
        time.sleep(RATE_LIMIT_DELAY)
