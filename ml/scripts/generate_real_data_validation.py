...
def fetch_options_rankings_with_returns(days_back: int = 120) -> pd.DataFrame:
    ...
    try:
        ...
        response = db.client.table("options_chain_snapshots") \
        ...
        df['contract_key'] = (
            df['underlying_symbol_id'].astype(str) + '_' +
            df['expiry'].astype(str) + '_' +
        ...
        df['days_diff'] = df.apply(
            lambda row: (row['next_date'] - row['snapshot_date']).days 
            if pd.notna(row['next_date']) else None, 
            axis=1
        )
        ...
        result = df[[
            'ranking_date', 'underlying_symbol', 'contract_key',
            'strike', 'side', 'expiry', 'mark', 'ml_score', 
            'next_mark', 'forward_return'
        ]].copy()
        ...
        if not result.empty:
            logger.info(f"Date range: {result['ranking_date'].min()} to {result['ranking_date'].max()}")
