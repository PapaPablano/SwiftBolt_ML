...
curl -s "https://cygflaemtmwiwaviclks.supabase.co/rest/v1/ohlc_bars_v2?symbol_id=eq.77e74624-6b80-430a-9543-328d41e52ce5&timeframe=eq.h1&provider=eq.alpaca&is_forecast=eq.false&select=ts&order=ts.desc&limit=10" \
  -H "Authorization: Bearer $ANON_KEY" \
  -H "apikey: $ANON_KEY" | jq '.[].ts'

echo ""
echo "Total Alpaca bars:"
curl -s "https://cygflaemtmwiwaviclks.supabase.co/rest/v1/ohlc_bars_v2?symbol_id=eq.77e74624-6b80-430a-9543-328d41e52ce5&timeframe=eq.h1&provider=eq.alpaca&is_forecast=eq.false&select=ts" \
  -H "Authorization: Bearer $ANON_KEY" \
